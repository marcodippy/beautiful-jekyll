...
Our `newComposedMonoid: Monoid[(Int, String)]` wouldn't work for this type, and when we try to compile `sum(anotherListOfTuples` it fails. We could write `implicit val anotherComposedMonoid: Monoid[(String, Int)] = composeMonoids(stringMonoid, intMonoid)`, but we don't want to create a new implicit val for every pair of monoids we want to compose (otherwise, why _monoid composition_ would be cool?). All we need to do is changing the signature of `composeMonoid` from 
```scala
def composeMonoids[A, B](ma: Monoid[A], mb: Monoid[B]) = new Monoid[(A, B)]
```
to
