...
More formally, a monoid is a **set of objects** (a _type_ to be precise) plus a binary **operation** to combine those objects that must obey the following laws:
...
* Identity Element: the operation must have an element (of the same set of the input, also called _zero_) that "does nothing": zero _op_ a == a and a _op_ zero == a (these laws are called, respectively, _left identity_ and _right identity_)
