...
### Why the closure law ???
Suppose that you want to calculate the sum of a list of integers List(1,2,3,4) and suppose that we start by summing 1 and 2 (remember that our operation is binary): 1+2 = 3 => List(3,3,4). Again: 3+3 = 6 => List(6,4). Again! 6+4 = 10. Great, we applied our "sum" monoid to the entire list and we got our result. If at one of this step the operation returned a Float instead of an Integer, our monoid would no longer fit the data (our monoid was supposed to sum _Integers_).
