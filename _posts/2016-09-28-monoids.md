...
```scala
case class Event(timestamp: LocalDate, eventType: String, revenue: BigDecimal, cost: BigDecimal)

val events = List(
  Event(LocalDate.of(2016, 6, 25), "event_A", 10, 5),
  Event(LocalDate.of(2016, 6, 25), "event_A", 20, 6),
  Event(LocalDate.of(2016, 9, 10), "event_B", 30, 7)
)
```
...
```
the _abstractions deamon_ inside us recognise a common pattern that we can factor out: we're always mapping over our `events` list to convert the event in something that has a monoid instance (`BigDecimal` first, `Stats` this time). So, it's time to create our generic `foldMap` function:
...
//in scalaz foldMap is defined on Foldable, so we could have written events.flatMap(eventToStat)
```
