...
* Identity Element: the operation must have an element (also called _zero_) that "does nothing": zero _op_ a == a and  
a _op_ zero == a (these laws are called, respectively, _left identity_ and _right identity_)
...
### Why the closure law ???
Suppose that you want to calculate the sum of a list of integers `List(1,2,3,4)` and suppose that we start by summing 1 and 2 (remember that our operation is binary): 1+2 = 3 => List(3,3,4).  
Again: 3+3 = 6 => List(6,4). Again! 6+4 = 10. Great, we applied our "sum" monoid to the entire list and we got our result. If at one of these steps the operation returned a Float instead of an Integer, our monoid would no longer be suitable for the task (our monoid knows how to sum _Integers_).
