...
Another good news is that we don't have to reinvent the wheel everytime we need a monoid, there are libraries that provide you all the infrastructure needed to work with this abstraction (and many others): the best known are [Cats](https://github.com/typelevel/cats) and [Scalaz](https://github.com/scalaz/scalaz). I'll use `scalaz` here to show some other use cases for monoids.
