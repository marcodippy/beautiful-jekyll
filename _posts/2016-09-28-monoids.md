...
Again!  
`6+4 = 10` 
Great, we applied our "sum" monoid to the entire list and we got our result. If at one of these steps the operation returned a Float instead of an Integer, our monoid would no longer be suitable for the task (our monoid only knows how to sum _Integers_).
